    ...
    namespace {
        ...
        public:
            ...
            double Evaluate(const SheetInterface& sheet) const override {
                ...
                case Multiply:
                    ...
                    break;
                case Divide:
                    if ((std::isfinite(lhs_value) && lhs_value == 0) 
                        || (std::isfinite(rhs_value) && rhs_value == 0))
                    {
        ...
        public:
            ...
            double Evaluate(const SheetInterface& sheet) const override {
               ...
               const auto node = sheet.GetCell(*cell_);
                ...
                CellInterface::Value result = node->GetValue();
                ...
                if (std::holds_alternative<double>(result)) {
                    ...
                    if (std::regex_match(str, std::regex("^[-+]?[0-9]*\\.?[0-9]+([eE][-+]?[0-9]+)?$"))) {
                        return std::stod(str);
                    }
                }
